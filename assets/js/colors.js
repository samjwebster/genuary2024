...
class Palette {
    ...
    makeMonochromatic(overwrite = false, focus=null, amount=0.5) {
        if(!focus) focus = floor(random()*this.size);
        if (overwrite == true) {
...
function allPalettes() {
    return [...basicBank(), ...porcelainBank(), ...complimentaryBank(), ...albersBank(), ...bauhausBank()];
}
